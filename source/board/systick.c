...
int systick_check_timer(systick_time_t *time_t)
{
	...
	if( systick_ms_overflow != time_t->systick_ms_overflow ){
		if( time_t->systick_ms > (u32)SYSTICK_MS_MAX ){
			time_t->systick_ms -= (u32)SYSTICK_MS_MAX;
		}else{
			//cause of check timer very late , so that you are here now.
			force_update = 1;
		}
		time_t->systick_ms_overflow = systick_ms_overflow;
	}		
