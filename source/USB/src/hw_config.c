...
void Set_System(void)
{
  ...
  //QUEUE_PacketCreate(&m_QueueUsbComTx, m_UsbComTxBuf, sizeof(m_UsbComTxBuf));
	...
	fifo_init( &usb_tx_fifo, m_UsbComTxBuf,  sizeof(m_UsbComTxBuf));
	fifo_init( &usb_rx_fifo, m_UsbComRxBuf,  sizeof(m_UsbComRxBuf));
...
uint32_t USB_RxWrite(uint8_t *buffter, uint32_t writeLen)
{
	...
	for( i=0; i< writeLen; i++){
		fifo_put( &usb_rx_fifo, buffter[i] );
	}
	return i;
