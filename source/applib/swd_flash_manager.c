...
#define util_assert(expression)  if( !(expression) ) flash_manager_printf("util_assert Error : %s , %d\n",__FILE__, __LINE__)
...
static bool page_erase_enabled = true;  //true : erase page  false : erase chip automatically 
...
static error_t setup_next_sector(uint32_t addr)
{
    ...
    if (page_erase_enabled) {
        ...
        flash_manager_printf("    intf->erase_sector(addr=0x%x) ret=%d\r\n", current_sector_addr,status);
        if (ERROR_SUCCESS != status) {
            intf->uninit();
            return status;
        }
